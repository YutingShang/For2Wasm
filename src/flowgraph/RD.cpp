...
RD::RD(BasicBlock* entryBasicBlock): entryBasicBlock(entryBasicBlock) {
    ...
    allDefinitions = AnalysisTools::getAllProgramDefinitions(entryNode);
    computeReachingDefinitions();    //will initialise the reachingDefs vector
 }

std::vector<std::unordered_map<std::string, std::set<std::string>>> RD::getReachingDefs() {
    for (int i = 0; i < basicBlocks.size(); i++) {
        std::cout << "-------------------------\nBasic block: " << basicBlocks[i]->getText() << std::endl;
        std::cout << "Reaching definitions: " << std::endl;
        for (const auto& [var, definitions] : reachingDefs[i]) {
...
std::vector<BasicBlock*> RD::getBasicBlocks() {
    return basicBlocks;
}

std::unordered_map<std::string, std::set<std::string>> RD::getAllDefinitions() {
...
void RD::computeReachingDefinitions() {
    ...
    reachingDefs = std::vector<std::unordered_map<std::string, std::set<std::string>>>(basicBlocks.size());
    ...
    while (changed) {
        ...
        for (int i = 0; i < basicBlocks.size(); i++) {     //still start from 0, process all basic blocks
            std::unordered_map<std::string, std::set<std::string>> new_reaches_set = basicBlockComputeReachingDefinitions(basicBlocks[i]);
...
std::unordered_map<std::string, std::set<std::string>> RD::basicBlockComputeReachingDefinitions(BasicBlock* basicBlock) {
    ...
    std::unordered_map<std::string, std::set<std::string>> reachesSet = {};
    ...
    for (BasicBlock* predecessor : predecessors) {
        ...
        std::unordered_map<std::string, std::set<std::string>> predecessorReachesSet = reachingDefs[predecessorIndex];
    ...
    for (auto it = instructions.begin(); it != instructions.end(); ++it) {
        ...
        reachesSet[definedVariable] = definedExpressions;
        }
    }
    return reachesSet;
}
