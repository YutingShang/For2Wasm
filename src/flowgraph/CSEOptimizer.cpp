...
bool CSEOptimizer::basicBlockRemoveCommonSubexpressions(BasicBlock *basicBlock, std::vector<BasicBlock*> &basicBlocks, std::vector<std::set<std::string>> &availSets, std::set<std::string> &allExpressions)
{
    ...
    for (auto it = instructions.begin(); it != instructions.end();) {
        ...
        if (in_avail_set.find(expr) != in_avail_set.end()) {    
            ...
            std::string tempVar = getNewProgramTempVariable(basicBlocks[0]);     //Also adds a declaration!!
            ...
            while (!toExploreStack.empty()) {
                ...
                if (current == basicBlock) {      //only for the first basic block, start backwards search from the current instruction
                    ...
                    expressionFound = basicBlockBackwardsFindAndReplaceExpression(current, basicBlocks, expr, tempVar, movNode);
                } else {             //for all other basic blocks, start backwards search from the end of the basic block
                    expressionFound = basicBlockBackwardsFindAndReplaceExpression(current, basicBlocks, expr, tempVar);
                }
...
bool CSEOptimizer::basicBlockBackwardsFindAndReplaceExpression(BasicBlock* basicBlock, std::vector<BasicBlock*> &basicBlocks, std::string &expressionToFind, std::string &tempVar, BaseNode* beginBackwardsFromThisNode)
{
...
std::string CSEOptimizer::getNewProgramTempVariable(BasicBlock* entryBasicBlock)
{
